...
//setub end
	while (true)
	{
		...
		{
		case NewGame:
			if(stateNewGame(true) == Restart) state = Restart; //(!Read_PB4)||(!Read_PE6)||(!Read_PE7)||(!Read_PE3)
			break;
		case Standard:
		...
		case Death:
			if(stateGameOver(snake_Length - startSize, true) == Restart) state = Restart; //(!Read_PB4)||(!Read_PE6)||(!Read_PE7)||(!Read_PE3)
			break;
		case Win:
			stateWin(snake_Length - startSize, true); //(!Read_PB4)||(!Read_PE6)||(!Read_PE7)||(!Read_PE3)
			state = Standard;
			break;
		case Restart:
...
void spawnApple(struct s_Cell* apple) //ПРАВИИИИТЬАААААААААААААААААА
{
	while (true)
	{
		int y = 1 + rand() % 6;
...
enum State stateStandart(struct s_Cell* snake, size_t* size, struct s_Cell* apple, char* dir)
{
	...
	//MCU_ADC_set_ch(4);
	//float ch_4_counts = MCU_ADC_read() - 38;
